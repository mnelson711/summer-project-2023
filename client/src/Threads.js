...
import {
    MDBContainer, MDBRow, MDBCol, MDBCard, MDBCardBody, MDBInput, MDBCheckbox, MDBIcon, MDBNavbarToggler, MDBNavbarLink, MDBCollapse, MDBNavbar, MDBNavbarItem, MDBNavbarBrand, MDBDropdown, MDBDropdownMenu, MDBDropdownToggle, MDBDropdownItem, MDBNavbarNav
...
export default function Threads() {
    ...
    const Insert = async () => {
        ...
        axios.post("http://localhost:3001/selectByThreadTitle", {
            title: Title,
        }).then((response) => {
            ...
            <MDBContainer fluid className='p-4 background-radial-gradient'>
                            ...
                            <MDBNavbarNav className='d-flex w-auto'>
                                <MDBNavbarItem className="margin">
                                    <MDBNavbarLink aria-current='page' href='/threads'>
                                        <MDBIcon fas icon='comments' size='2x' className='text-black' />
                                    </MDBNavbarLink>
                                </MDBNavbarItem>
                                <MDBNavbarItem className="margin">
                ...
                {showAddThread ?
                    <MDBCard id="addThreadMenu" className={fade}>
                        <MDBCardBody>
                            <MDBInput label='Title' placeholder="Title" id='title' type='text' onChange={(e) => { setTitle(e.target.value); }} />
                            {TitleTaken ? <p className="mb-3 mt-4"><Alert className="fade-out alert" severity="warning">Thread name taken!</Alert></p> : null}

                            <MDBInput label='Description' placeholder="Description" id='description' type='text' onChange={(e) => { setDescription(e.target.value); }} />
                            <MDBRow>
                                <div className="flex2">
                                    <MDBInput label='Tags' placeholder="Tags" id='tags' type='text' onChange={(e) => { setIndividualTag(e.target.value); }} />
                                </div>
                                <div className="flex1">
                                    <button className="bg-glass addTagButton" onClick={addTag} >Add</button>
                                </div>
                                <div className="flex2">
                                    <div className="tags">
                                        {tags.map((tag) => (
                            ...
                            <MDBRow style={{ marginTop: "1%" }}>
                                <div className="flex1">
                                    <button className="createThreadButton" onClick={Insert}>Create</button>
                                </div>
                                <div className="flex1">
                                    <button className="cancelButton" onClick={handleshowAddThread}>Cancel</button>
                                </div>
                            </MDBRow>
                        </MDBCardBody>
                    </MDBCard> : null}
                ...
                {threadCreated ? <p className="mb-3 mt-4"><Alert className="fade-out alert" severity="success">Thread created!</Alert></p> : null}
